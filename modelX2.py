...
class SDM(nn.Module):
    def __init__(self, in_channels, num_clusters, batchsize):
        ...
        # self.CDF_2 = DFM(in_channels=self.in_channels // 6, num_cluster=self.num_clusters, batchsize=self.batchsize)
        # self.CDF_3 = DFM(in_channels=self.in_channels // 6, num_cluster=self.num_clusters, batchsize=self.batchsize)
        # self.CDF_4 = DFM(in_channels=self.in_channels // 6, num_cluster=self.num_clusters, batchsize=self.batchsize)
        # self.CDF_5 = DFM(in_channels=self.in_channels // 6, num_cluster=self.num_clusters, batchsize=self.batchsize)
        # self.CDF_6 = DFM(in_channels=self.in_channels // 6, num_cluster=self.num_clusters, batchsize=self.batchsize)
    ...
    def forward(self, LR):
        ...
        # max_pooling = torch.max(LR_filter, dim=1, keepdim=True)[0]
        # max_pool = max_pooling.repeat_interleave(self.num_clusters, dim=1)

        # HR = self.c1_c(torch.cat((self.SD_1(LR_filter), self.SD_2(LR_filter, max_pool)), dim=1))
        HR = self.c1_c(self.SD_1(LR_filter))

        # LR_split_1, LR_split_2, LR_split_3, LR_split_4, LR_split_5, LR_split_6, = torch.chunk(LR, chunks=6, dim=1)
        # LR_split_1, LR_split_2,  = torch.chunk(LR, chunks=2, dim=1)

        LR_fuse_1 = self.CDF_1(HR, LR) + LR
        # LR_fuse_2 = self.CDF_2(HR, LR_split_2) + LR_split_2
        # LR_fuse_3 = self.CDF_3(HR, LR_split_3) + LR_split_3
        # LR_fuse_4 = self.CDF_4(HR, LR_split_4) + LR_split_4
        # LR_fuse_5 = self.CDF_5(HR, LR_split_5) + LR_split_5
        # LR_fuse_6 = self.CDF_6(HR, LR_split_6) + LR_split_6

        # LR_fuse = torch.cat([LR_fuse_1, LR_fuse_2, LR_fuse_3, LR_fuse_4, LR_fuse_5, LR_fuse_6], dim=1)
        # LR_fuse = torch.cat([LR_fuse_1, LR_fuse_2, ], dim=1)
...
class main(nn.Module):
    ...
    def forward(self, LR):
        ...
        # LR_out_6 = self.SDM6(out_1) + out_1
        # out_6 = self.RBS[5](LR_out_6)
        ...
        # LR_out_3 = self.SDM3(out_2) + out_2
        # out_3 = self.RBS[2](LR_out_3)

        # out_3_up = self.upsampleX2(out_2) + LRX4
        #
        # LR_out_4 = self.SDM4(out_3_up) + out_3_up
        # out_4 = self.RBS[3](LR_out_4)

        # out_4_up = self.upsampleX2(out_4) + LRX8
        #
        # LR_out_5 = self.SDM5(out_4_up) + out_4_up
        # final_out= self.RBS[4](LR_out_5)



        final_out = self.final_conv(out_2) + LRX2

        return final_out, out_1